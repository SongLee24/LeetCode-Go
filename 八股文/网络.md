...
关于ARQ协议：由于**停止等待ARQ协议**的信道利用率太低，所以采用**连续ARQ协议**。连续ARQ协议通常是结合滑动窗口协议来使用的，位于窗口内的多个分组都可以连续发送出去，而不需要等待对方的确认，这样就提高了信道利用率。参考：https://blog.csdn.net/guoweimelon/article/details/50879588
