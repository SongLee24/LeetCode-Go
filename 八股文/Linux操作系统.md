...
![](https://github.com/SongLee24/LeetCode-Go/blob/main/%E5%85%AB%E8%82%A1%E6%96%87/images/linux-thread-sync.jpg?raw=true)

### 12、WRR算法进行流量分发

原始的WRR算法：假设有３台机器ａ，ｂ，ｃ权重分别为５,１,１，原始的WRR算法实现是：每次选出权重最大的，给其权重减1,直到权重全部为0后，按照此时生成的序列轮训。
...
最终确定的轮序序列是`a,a,a,a,a,b,c`，虽然此时负载是均衡的(5:1:1)，但是前五次都去访问了a机器，b和c机器是空闲的，这样并不是最好的，理想状态是在均衡的情况下让访问次数也均衡开，于是就有了SWRR（Smooth Weighted Round Robin）,平滑加权轮训调度算法。
...
参考：https://blog.csdn.net/yangbodong22011/article/details/73369426
